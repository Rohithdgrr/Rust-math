...
- Domain verbs are consistent: `area()`, `perimeter()`, `volume()`,
  `distance()`, `intersect()`, `solve()`, `integrate()`, `differentiate()`,
  `transform()`.
...
- Prefer `T: Real`/`T: Field` style bounds; never pin to `f64` in public APIs
  unless the algorithm requires it (and say so in docs).
- Generic functions must work for `f32`, `f64`, and (where applicable)
  fixed-point/arbitrary-precision types.
...
Complex configuration (e.g., plot figures, optimizers, filters) uses builder
patterns:
...
- Property tests: generic numeric code (identity laws, commutativity where
  applicable).
- Numerical accuracy: compare against closed-form references with documented
  tolerances.
