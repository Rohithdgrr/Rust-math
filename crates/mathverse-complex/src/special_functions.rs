...
impl ComplexSpecialFunctions {
    ...
    fn zeta_euler_maclaurin(s: Complex, n_terms: usize) -> Complex {
        ...
        sum = sum + n_c.pow(Complex::one() - s) / (s - Complex::one()) + n_c.pow(-s) / Complex::real(2.0);
    ...
    pub fn airy(z: Complex, iterations: usize) -> (Complex, Complex) {
        if z.norm() < 1e-15 {
            ...
            let c1 = Complex::real(3.0_f64.powf(-2.0 / 3.0)) / Self::gamma(Complex::real(2.0 / 3.0));
        ...
        if z.norm() < 2.0 {
            ...
            let ai = (-t).exp() / (Complex::real(2.0) * Complex::real(std::f64::consts::PI.sqrt()) * z_pow_quarter);
    ...
    pub fn bessel_j(v: Complex, z: Complex, iterations: usize) -> Complex {
        if z.norm() < 1e-15 {
            return if v == Complex::zero() { Complex::one() } else { Complex::zero() };
        }
        if z.norm() < 10.0 {
            ...
            for n in 0..=iterations {
                let n_complex = Complex::real(n as f64);
                let factorial_n: f64 = if n == 0 { 1.0 } else { (1..=n).map(|x| x as f64).product() };
            ...
            // (complex sqrt so the exponential decay/growth sectors come out right)
            let sqrt_term = (Complex::real(2.0) / (Complex::real(std::f64::consts::PI) * z)).sqrt();
            let phase = z - v * Complex::real(std::f64::consts::FRAC_PI_2) - Complex::real(std::f64::consts::FRAC_PI_4);
    ...
    fn bessel_y_integer(n: usize, z: Complex, iterations: usize) -> Complex {
        ...
        for k in 0..n {
            ...
            sum1 = sum1 + Complex::real(fact_n_k_1 / fact_k)
                * z_over_2.pow(Complex::real((2 * k) as f64 - n as f64));
        }

        let mut sum2 = Complex::zero();
        for k in 0..=iterations {
            let sign = if k % 2 == 0 { Complex::one() } else { -Complex::one() };
            let fact_k: f64 = (1..=k).map(|x| x as f64).product();
            ...
            sum2 = sum2 + sign * psi / Complex::real(fact_k * fact_k_n)
                * z_over_2.pow(Complex::real((2 * k + n) as f64));
        }

        Complex::real(2.0 / pi) * z_over_2.ln() * j_n
            - Complex::real(1.0 / pi) * (sum1 + sum2)
    }
}
