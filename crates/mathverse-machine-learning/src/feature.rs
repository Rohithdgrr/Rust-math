...
pub fn min_max(x: &mut [Vec<f64>]) -> (Vec<f64>, Vec<f64>) {
    ...
    for row in x.iter_mut() {
        for j in 0..p {
            ...
            row[j] = if range.abs() < 1e-10 { 0.0 } else { (row[j] - mins[j]) / range };
        }
    }
    (mins, maxs)
}
...
pub fn one_hot(labels: &[f64], num_classes: usize) -> Vec<Vec<f64>> {
    labels.iter().map(|&l| {
        let mut row = vec![0.0; num_classes];
        let idx = l as usize;
        if idx < num_classes { row[idx] = 1.0; }
        row
    }).collect()
}
...
pub fn one_hot_decode(encoded: &[Vec<f64>]) -> Vec<f64> {
    encoded.iter().map(|row| {
        row.iter().enumerate().max_by(|a, b| a.1.partial_cmp(b.1).unwrap()).map(|(i, _)| i as f64).unwrap_or(0.0)
    }).collect()
}
