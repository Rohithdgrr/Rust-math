...
fn init_centroids(x: &[Vec<f64>], k: usize) -> Vec<Vec<f64>> {
    ...
    for _ in 1..k {
        let dists: Vec<f64> = x.iter().map(|xi| {
            centroids.iter().map(|c| euclidean(xi, c)).fold(f64::INFINITY, f64::min)
        }).collect();
        ...
        if !pushed { centroids.push(x[0].clone()); }
    }
    centroids
}
    ...
    for iter in 0..max_iters {
        ...
        for i in 0..n {
            let best = centroids.iter().enumerate()
                .min_by(|a, b| euclidean(&x[i], a.1).partial_cmp(&euclidean(&x[i], b.1)).unwrap())
                .unwrap().0;
        ...
        for i in 0..n {
            counts[labels[i]] += 1;
            for j in 0..p { new_centroids[labels[i]][j] += x[i][j]; }
        }
        for c in 0..k {
            if counts[c] > 0 {
                for j in 0..p { new_centroids[c][j] /= counts[c] as f64; }
            } else {
                new_centroids[c] = centroids[c].clone();
            }
        }
        let shift: f64 = centroids.iter().zip(&new_centroids)
            .map(|(old, new)| euclidean(old, new)).sum();
        centroids = new_centroids;
        if !changed || shift < tol {
            let inertia: f64 = (0..n).map(|i| euclidean(&x[i], &centroids[labels[i]]).powi(2)).sum();
            return Ok(KMeansResult { centroids, labels, inertia, n_iters: iter + 1 });
        }
    }
    let inertia: f64 = (0..n).map(|i| euclidean(&x[i], &centroids[labels[i]]).powi(2)).sum();
    Ok(KMeansResult { centroids, labels, inertia, n_iters: max_iters })
}
