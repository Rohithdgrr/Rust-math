...
pub fn fit(x: &[Vec<f64>], y: &[f64], lr: f64, max_iters: usize, tol: f64) -> MathResult<LogisticResult> {
    let n = y.len();
    let p = x[0].len();
    ...
    for iter in 0..max_iters {
        ...
        if max_change < tol { return Ok(LogisticResult { coefficients: coef, intercept, n_iters: iter + 1 }); }
    }
    Ok(LogisticResult { coefficients: coef, intercept, n_iters: max_iters })
}

/// Predict probabilities.
pub fn predict_proba(x: &[Vec<f64>], coefficients: &[f64], intercept: f64) -> Vec<f64> {
    x.iter().map(|row| {
        let z = intercept + row.iter().zip(coefficients).map(|(xi, ci)| xi * ci).sum::<f64>();
        sigmoid(z)
    }).collect()
}
...
pub fn predict(x: &[Vec<f64>], coefficients: &[f64], intercept: f64) -> Vec<f64> {
    predict_proba(x, coefficients, intercept).iter().map(|&p| if p >= 0.5 { 1.0 } else { 0.0 }).collect()
}
