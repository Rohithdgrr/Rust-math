...
pub fn pearson_correlation(a: &[f64], b: &[f64]) -> f64 {
    ...
    if sa < 1e-15 || sb < 1e-15 { 0.0 } else { cov / (sa * sb / n) }
}
...
fn discretize(values: &[f64], n_bins: usize) -> Vec<usize> {
    ...
    let range = max - min;
    if range < 1e-15 { return vec![0; values.len()]; }
    values.iter().map(|&v| {
...
pub fn select_k_best(x: &[Vec<f64>], y: &[f64], k: usize) -> (Vec<usize>, Vec<Vec<f64>>) {
    ...
    let mut scores: Vec<(usize, f64)> = (0..p).map(|j| {
