...
pub fn fit_elastic_net(
    ...
    let col_mean: Vec<f64> = (0..p)
        .map(|j| x.iter().map(|xi| xi[j]).sum::<f64>() / n as f64)
        .collect();
    ...
    let col_norm2: Vec<f64> = (0..p)
        .map(|j| x_centered.iter().map(|xi| xi[j] * xi[j]).sum::<f64>())
        .collect();
...
pub fn predict(x: &[Vec<f64>], coefficients: &[f64], intercept: f64) -> Vec<f64> {
    x.iter()
        .map(|xi| xi.iter().zip(coefficients).map(|(v, c)| v * c).sum::<f64>() + intercept)
        .collect()
}
