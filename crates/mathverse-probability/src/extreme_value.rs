...
impl GumbelEVD {
    ...
    pub fn mean(&self) -> f64 {
        self.location + self.scale * 0.57721566490153286060651209008240243104215933593992f64
    }
...
impl GEVDistribution {
    ...
    pub fn mean(&self) -> f64 {
        ...
        if xi < 1.0 && xi != 0.0 {
            ...
            mu + sigma * 0.57721566490153286060651209008240243104215933593992f64
        } else {
            f64::INFINITY
        }
    }
...
impl ReturnPeriod {
    ...
    pub fn return_period_for_value(
        gev: &GEVDistribution,
        value: f64,
    ) -> f64 {
        let p = gev.cdf(value);
...
impl PeakOverThreshold {
    pub fn new(threshold: f64, data: Vec<f64>) -> Self {
        let excesses: Vec<f64> = data.into_iter()
    ...
    pub fn fit_gpd(&self) -> Result<(f64, f64), String> {
        ...
        let shape = -0.5 * (1.0 - mean * mean / variance).signum() 
            * (1.0 - mean * mean / variance).sqrt();
...
impl BlockMaxima {
    ...
    pub fn compute(&self) -> Vec<f64> {
        let n_blocks = (self.data.len() + self.block_size - 1) / self.block_size;
        ...
        for i in 0..n_blocks {
            ...
            if start < self.data.len() {
                let block_max = self.data[start..end].iter().cloned()
    ...
    pub fn fit_gev(&self) -> Result<(f64, f64, f64), String> {
        ...
        let variance = maxima.iter()
            .map(|&x| (x - mean).powi(2))
            .sum::<f64>() / (maxima.len() - 1) as f64;
        
        let skewness = maxima.iter()
            .map(|&x| (x - mean).powi(3))
            .sum::<f64>() / maxima.len() as f64 / variance.powf(1.5);
        
        // Simplified parameter estimation
        let shape = -0.1 * skewness.signum();  // Very rough approximation
        let scale = variance.sqrt();
        let location = mean - scale * 0.57721566490153286060651209008240243104215933593992f64;
                ...
                if let Some(last_idx) = last_exceedance_idx {
                    if i - last_idx >= min_gap {
                        if !current_cluster.is_empty() {
                            clusters.push(current_cluster.iter().cloned().fold(f64::NEG_INFINITY, f64::max));
...
mod tests {
    ...
    fn test_gumbel_evd() {
        ...
        let cdf = gev.cdf(0.0);
        assert!((cdf - (-1.0_f64).exp().exp()).abs() < 1e-10);
    }
