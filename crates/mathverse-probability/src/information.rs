...
impl Entropy {
    ...
    pub fn renyi(probabilities: &[f64], alpha: f64) -> f64 {
        ...
        let sum: f64 = probabilities.iter()
            .filter(|&&p| p > 0.0)
            .map(|&p| p.powf(alpha))
            .sum();
...
impl InformationBottleneck {
    ...
    pub fn optimize(
        ...
        for _ in 0..iterations {
            ...
            let mut p_x: Vec<f64> = (0..n_x).map(|i| joint_xy[i].iter().sum()).collect();
            ...
            for i in 0..n_x {
                ...
                let mut exp_weights: Vec<f64> = weights.iter().map(|&w| (w - max_weight).exp()).collect();
