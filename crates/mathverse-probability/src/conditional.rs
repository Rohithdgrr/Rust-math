...
impl ConditionalProbability {
    ...
    pub fn extended_bayes(
        ...
        let p_b: f64 = p_b_given_a.iter().zip(p_a.iter())
            .map(|(&pba, &pa)| pba * pa)
            .sum();
...
impl ConditionalDistributions {
    ...
    pub fn conditional_pmf(joint_pmf: &[Vec<f64>], x: usize, y: usize) -> f64 {
        let p_xy = joint_pmf.get(x).and_then(|row| row.get(y)).copied().unwrap_or(0.0);
        
        let p_y: f64 = joint_pmf.iter()
...
impl ConditionalExpectation {
    ...
    pub fn total_variance(
        ...
        let var_exp: f64 = expectations.iter()
...
impl IndependenceTesting {
    ...
    pub fn variables_independent_discrete(
        joint_pmf: &[Vec<f64>],
        tolerance: f64,
    ) -> bool {
    ...
    pub fn mutual_information_test(
        joint_pmf: &[Vec<f64>],
        threshold: f64,
    ) -> bool {
...
impl Martingales {
    ...
    pub fn optional_stopping(
        ...
        for (i, &t) in stopping_times.iter().enumerate() {
...
impl BayesianNetwork {
    ...
    pub fn variable_elimination(
        ...
        let mut factors = cpts.to_vec();
        
        for &var in elimination_order {
            if var != query_var {
                // Sum out variable (placeholder)
                factors.retain(|f| true);
            }
        }
        ...
        for cpt in &factors {
            ...
            for (i, &(var, val)) in evidence.iter().enumerate() {
                assignment[i] = val;
            }
...
mod tests {
    ...
    fn test_bivariate_normal_conditional() {
        let (mean, var) = ConditionalDistributions::bivariate_normal_conditional(0.0, 0.0, 1.0, 1.0, 0.5, 1.0);
