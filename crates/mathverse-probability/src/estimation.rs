    ...
    pub fn general(
        ...
        for _ in 0..1000 {
            ...
            for p in &mut new_params {
                *p += crate::distributions::Normal { mu: 0.0, sigma: 0.01 }.sample(&mut crate::rng::Rng::new(42));
            }
...
impl MethodOfMoments {
    ...
    pub fn general(
        data: &[f64],
        moment_equations: impl Fn(&[f64], &[f64]) -> Vec<f64>,
        n_moments: usize,
    ) -> Vec<f64> {
        let n = data.len();
...
impl EstimatorProperties {
    ...
    pub fn bias(
        ...
        let estimates: Vec<f64> = samples.iter()
            .map(|sample| estimator(sample))
            .collect();
    ...
    fn generate_samples(n: usize, n_samples: usize, true_param: f64) -> Vec<Vec<f64>> {
        ...
        (0..n_samples)
            .map(|_| {
                (0..n)
                    .map(|_| true_param + crate::distributions::Normal { mu: 0.0, sigma: 1.0 }.sample(&mut rng))
                    .collect()
            })
            .collect()
    }
...
impl CramerRaoBound {
    ...
    pub fn fisher_information(
        ...
        let first_derivative = (ll_plus - ll_minus) / (2.0 * epsilon);
        
        let ll_plus_plus = log_likelihood(parameter + epsilon, 0.0);
        let ll_plus_minus = log_likelihood(parameter - epsilon, 0.0);
        let second_derivative = (ll_plus_plus - 2.0 * log_likelihood(parameter, 0.0) + ll_plus_minus) / (epsilon * epsilon);
...
impl BayesianEstimation {
    ...
    pub fn posterior_mean(
        ...
        // Simplified: assume scalar data precision
        for i in 0..n {
            posterior_mean[i] = prior_mean[i];  // Placeholder
        }
