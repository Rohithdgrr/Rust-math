...
impl RatioOfRandomVariables {
    ...
    pub fn normal_ratio(mean1: f64, sigma1: f64, mean2: f64, sigma2: f64) -> (f64, f64) {
        if mean2 != 0.0 {
            ...
            let var_ratio = (sigma1 * sigma1 / mean2.powi(2) 
                + mean1 * mean1 * sigma2 * sigma2 / mean2.powi(4));
    ...
    pub fn general_ratio(
        mean1: f64, var1: f64,
        mean2: f64, var2: f64,
    ) -> (f64, f64) {
        if mean2 != 0.0 {
            let mean_ratio = mean1 / mean2;
            let var_ratio = (var1 / mean2.powi(2) 
                + mean1 * mean1 * var2 / mean2.powi(4));
            (mean_ratio, var_ratio)
        } else {
            (0.0, f64::INFINITY)
        }
...
impl MixtureDistribution {
    ...
    pub fn two_component(
        weight: f64,
        mean1: f64, var1: f64,
        mean2: f64, var2: f64,
    ) -> (f64, f64) {
        let mean = weight * mean1 + (1.0 - weight) * mean2;
        let variance = weight * (var1 + mean1 * mean1) 
            + (1.0 - weight) * (var2 + mean2 * mean2) 
            - mean * mean;
        (mean, variance)
    }

    /// Mixture of multiple distributions.
    pub fn multiple_component(
        weights: &[f64],
        means: &[f64],
        variances: &[f64],
    ) -> (f64, f64) {
        let mean: f64 = weights.iter().zip(means.iter())
            .map(|(&w, &m)| w * m)
            .sum();
        
        let variance: f64 = weights.iter().zip(means.iter()).zip(variances.iter())
            .map(|(&w, &m, &v)| w * (v + m * m))
            .sum() - mean * mean;
    ...
    pub fn pdf(
        x: f64,
        weights: &[f64],
        pdfs: &[Box<dyn Fn(f64) -> f64>],
    ) -> f64 {
        weights.iter().zip(pdfs.iter())
            .map(|(&w, pdf)| w * pdf(x))
            .sum()
    }
    ...
    pub fn cdf(
        x: f64,
        weights: &[f64],
        cdfs: &[Box<dyn Fn(f64) -> f64>],
    ) -> f64 {
        weights.iter().zip(cdfs.iter())
            .map(|(&w, cdf)| w * cdf(x))
            .sum()
    }
}
...
impl RandomVariableAlgebra {
    ...
    pub fn weighted_sum(
        ...
        let mean: f64 = means.iter().zip(weights.iter())
            .map(|(&m, &w)| w * m)
            .sum();
        
        let variance: f64 = variances.iter().zip(weights.iter())
            .map(|(&v, &w)| w * w * v)
            .sum();
...
impl RandomVariableTransform {
    ...
    pub fn probability_integral_transform(
        cdf: impl Fn(f64) -> f64,
        x: f64,
    ) -> f64 {
        cdf(x)
    ...
    pub fn inverse_probability_integral_transform(
