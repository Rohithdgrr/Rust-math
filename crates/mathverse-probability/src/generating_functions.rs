...
impl ProbabilityGeneratingFunction {
    ...
    pub fn variance_from_pgfd(pmf: &[f64]) -> f64 {
        ...
        let second_derivative = (g_plus_plus - 2.0 * Self::compute(pmf, 1.0) + g_minus_minus) / (4.0 * epsilon * epsilon);
...
impl LaplaceTransform {
    ...
    pub fn inverse(L: impl Fn(f64) -> f64, t: f64, sigma: f64, n: usize) -> f64 {
        let omega = 10.0;
...
impl ZTransform {
    ...
    pub fn inverse(X: impl Fn(f64) -> f64, n: usize) -> Vec<f64> {
        let mut sequence = Vec::with_capacity(n);
        
        for k in 0..n {
            let integral = Self::inverse_integral(&X, k as f64);
    ...
    fn inverse_integral(X: impl Fn(f64) -> f64, k: f64) -> f64 {
        let n = 1000;
...
impl BivariateGeneratingFunctions {
        ...
        joint_pmf.iter()
            ...
            .flat_map(|(i, row)| {
                row.iter().enumerate().map(move |(j, &p)| {
                    p * s.powi(i as i32) * t.powi(j as i32)
                })
            })
            .sum()
    }
    ...
    pub fn covariance_from_pgf(joint_pmf: &[Vec<f64>]) -> f64 {
        ...
        let g_11 = Self::joint_pgf(joint_pmf, 1.0, 1.0);
        let g_10 = Self::joint_pgf(joint_pmf, 1.0, 0.0);
        let g_01 = Self::joint_pgf(joint_pmf, 0.0, 1.0);
        
        let mean_x = (Self::joint_pgf(joint_pmf, 1.0 + epsilon, 1.0) 
            - Self::joint_pgf(joint_pmf, 1.0 - epsilon, 1.0)) / (2.0 * epsilon);
        let mean_y = (Self::joint_pgf(joint_pmf, 1.0, 1.0 + epsilon) 
            - Self::joint_pgf(joint_pmf, 1.0, 1.0 - epsilon)) / (2.0 * epsilon);
        
        let mean_xy = (Self::joint_pgf(joint_pmf, 1.0 + epsilon, 1.0 + epsilon) 
            - Self::joint_pgf(joint_pmf, 1.0 - epsilon, 1.0 + epsilon)
            - Self::joint_pgf(joint_pmf, 1.0 + epsilon, 1.0 - epsilon)
            + Self::joint_pgf(joint_pmf, 1.0 - epsilon, 1.0 - epsilon)) / (4.0 * epsilon * epsilon);
